...
try {
  ...
  while (Date.now()-start < 8000) {
    ...
    if (!isProcessAlive(info.workerPid) && events.includes(`guardian-cleaned:${info.workerPid}`)) break;
    await delay(50);
  }
  assert(!isProcessAlive(info.workerPid), 'worker survived host crash');
  ...
  if (isProcessAlive(info.workerPid)) process.kill(info.workerPid,'SIGKILL');
  if (isProcessAlive(info.guardianPid)) process.kill(info.guardianPid,'SIGKILL');
