...
import { SNAPSHOT_PROBE_SYSTEM_PROMPT } from '../../prompts/snapshot-probe.js';
...
if (!['http:', 'https:'].includes(origin.protocol) || !['127.0.0.1', 'localhost', '[::1]'].includes(origin.hostname) || origin.username || origin.password || origin.pathname !== '/' || origin.search || origin.hash) throw new Error(ERROR_CODES.INVALID_LIVE_ORIGIN);
...
async function readJson(response: Response) {
  if (!response.ok || !response.body) throw new Error(ERROR_CODES.LIVE_HTTP_FAILED);
  let bytes = 0;
  const chunks: Uint8Array[] = [];
  for await (const chunk of response.body) {
    bytes += chunk.length;
    if (bytes > 128 * 1024) throw new Error(ERROR_CODES.LIVE_RESPONSE_LIMIT);
    chunks.push(chunk);
  }
...
async function chat(messages: Message[], withTools: boolean) {
  ...
  if (filter.filter(reply.message.content).redacted) throw new Error(ERROR_CODES.SENSITIVE_MODEL_OUTPUT);
...
try {
  ...
  if (!installed) throw new Error(ERROR_CODES.LIVE_MODEL_NOT_INSTALLED);
  await git('init');
  ...
  const messages: Message[] = [
    { role: 'system', content: SNAPSHOT_PROBE_SYSTEM_PROMPT },
    { role: 'user', content: 'Read facts.txt and report SNAPSHOT_VALUE.' },
  ];
  const first = await chat(messages, true);
  if (first.tool_calls?.length !== 1) throw new Error(ERROR_CODES.LIVE_EXPECTED_ONE_TOOL_CALL);
  ...
  if (second.tool_calls?.length || second.content.trim() !== '731') throw new Error(ERROR_CODES.LIVE_SNAPSHOT_ANSWER_FAILED);
