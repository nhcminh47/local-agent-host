...
const daemonPath = fileURLToPath(new URL('../../runtime/daemon.js', import.meta.url));
const bridgePath = fileURLToPath(new URL('../../runtime/mcp-server.js', import.meta.url));
...
async function startDaemon() {
  ...
  const port = await new Promise<number>((resolve, reject) => {
    let text = '';
    const timeout = setTimeout(() => reject(new Error(ERROR_CODES.DAEMON_START_TIMEOUT)), 10_000);
    daemon!.once('error', () => { clearTimeout(timeout); reject(new Error(ERROR_CODES.DAEMON_START_FAILED)); });
    daemon!.once('exit', () => { clearTimeout(timeout); reject(new Error(ERROR_CODES.DAEMON_EXITED)); });
    daemon!.stdout!.on('data', chunk => {
      text += String(chunk);
      if (text.length > 16_000) { clearTimeout(timeout); reject(new Error(ERROR_CODES.DAEMON_OUTPUT_LIMIT)); return; }
...
try {
  if (live) {
    ...
    if (!modelToken) throw new Error(ERROR_CODES.LIVE_CREDENTIAL_UNAVAILABLE);
  } else {
  ...
  if (!live) {
    const cases = [
      ['http', ERROR_CODES.EXPLORER_HTTP_FAILED], ['json', ERROR_CODES.EXPLORER_INVALID_RESPONSE],
      ['schema', ERROR_CODES.EXPLORER_INVALID_RESPONSE], ['oversized', ERROR_CODES.EXPLORER_RESPONSE_LIMIT],
      ['disconnect', ERROR_CODES.EXPLORER_TRANSPORT_FAILED], ['tool-input', 'EXPLORER_FAILED'],
      ['output', ERROR_CODES.EXPLORER_INVALID_COMPLETION],
    ] as const;
