...
export class DaemonClient {
  ...
  constructor(origin: string, token: string) {
    ...
    if (this.#origin.protocol !== 'http:' || !['127.0.0.1', 'localhost', '[::1]'].includes(this.#origin.hostname) || this.#origin.username || this.#origin.password || this.#origin.pathname !== '/') throw new Error(ERROR_CODES.INVALID_DAEMON_ORIGIN);
