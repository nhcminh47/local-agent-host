...
async function chat(model: string, messages: Message[], withTools: boolean) {
  ...
  const consume = (line: string) => {
    ...
    if (chunk.error) throw new Error('MODEL_RESPONSE_ERROR');
  ...
  for await (const chunk of response.body) {
    ...
    if (bytes > 2 * 1024 * 1024) throw new Error('RESPONSE_LIMIT');
    buffer += decoder.decode(chunk, { stream: true });
    let newline: number;
    while ((newline = buffer.indexOf('\n')) !== -1) { consume(buffer.slice(0, newline)); buffer = buffer.slice(newline + 1); }
  }
  consume(buffer + decoder.decode());
  if (!final) throw new Error('INCOMPLETE_STREAM');
...
for (const model of models) {
  try {
    ...
    if (calls.length !== 1) throw new Error('EXPECTED_ONE_TOOL_CALL');
    Tool.parse(calls[0]);
