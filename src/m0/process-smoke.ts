...
try {
  ...
  while ([...pids].some(alive) && Date.now() - start < 5000) await delay(50);
  assert.deepEqual([...pids].filter(alive), [], 'Fixture descendants must all terminate');
  ...
  if (root.pid && alive(root.pid)) await terminateTree(root.pid).catch(() => undefined);
